<%@ page language="java" contentType="text/html; charset=UTF-8"
		 pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
	<meta charset="UTF-8">
	<title>天高报名表</title>
	<style>
		h1{
			text-align: center;
			font-weight: bolder;
		}
		img{
			float: right;
			display: block;
			margin: 5px;
			width: 280px;
			height: 400px;
			/*border:1px solid blue;*/
		}
		.content{  //内容框
		border: 30px solid red;
			background-color: white;
		}
		.short{
			display: inline-block;
			line-height:55px;
			font-size: 22px;
			font-weight: bold;
			text-align: left;
		}
		#content{  //姓名框
		display: block;
			background-color: white;
		}
		.bigcontent{
			/*background-color: gray;*/
			display: block;
			position: relative;
			top: 25px;
		}
		.name, .grade, .majorClass, .born, .hobby, .phone, .choose{
			display: inline-block;
			font-size: 20px;
		}
		.resume,.experience,.futurePlan{
			display: inline-block;
			width:850px;
			padding-top:6px;
			line-height:32px;
			vertical-align: top;
			word-break: break-all;
			font-size: 22px;
		}
		.button{
			text-align: center;
			position: relative;
			top: 30px;
			left: -10px;
		}
		.center{
			width: 860px;
			position: relative;
			left: 20%;
		}
	</style>
</head>
<body>

<br/>
<div class="center">
	<br/>
	<br/>
	<h1>天高报名表</h1>
	<div class="content">
		<img src="${pageContext.request.contextPath }/res/img/Btn_picture_nor@2x.png" alt="">
		<div class="short" id="content">
			姓名：<span class="name"></span><br />
			年级：<span class="grade"></span><br />
			专业班级：<span class="majorClass"></span><br />
			出生年月：<span class="born"></span><br />
			兴趣爱好：<span class="hobby"></span><br />
			手机号码：<span class="phone"></span><br />
			技术意向：<span class="choose"></span><br />

		</div>
		<div class="bigcontent">
			<span class="short">个人简介&技能：</span>
			<span class="resume"></span><br />
			<span class="short">项目&工作经历：</span>
			<span class="experience"></span><br />
			<span class="short">未来规划：</span>
			<span class="futurePlan"></span>
		</div>
	</div>
	<div class="button">
		<input type="button" value="上一个" id="return">
		<input type="button" value="下一个" id="next">
		<input type="button" value="打印" id="print" onclick="download()">
	</div>
</div>
<script src="${pageContext.request.contextPath }/res/js/jquery-1.8.0.min.js"></script>
<script src="js/new_file.js"></script>
<script>
	window.onload = function(){
		var page = 0;
		var img = document.getElementsByTagName("img")[0];
		var name = document.getElementsByClassName("name")[0];
		var grade = document.getElementsByClassName("grade")[0];
		var majorClass = document.getElementsByClassName("majorClass")[0];
		var born = document.getElementsByClassName("born")[0];
		var hobby = document.getElementsByClassName("hobby")[0];
		var phone = document.getElementsByClassName("phone")[0];
		var choose = document.getElementsByClassName("choose")[0];
		var resume = document.getElementsByClassName("resume")[0];
		var experience = document.getElementsByClassName("experience")[0];
		var futurePlan = document.getElementsByClassName("futurePlan")[0];
		var btn = document.getElementById("next");
		var pre = document.getElementById("return");
		var pri = document.getElementById("print");

		createInfo(page);

		btn.onclick=function(){
			page++;
			//调用接口
			createInfo(page);
		}
		pre.onclick=function(){
			if (page > 0) {
				page--;
				createInfo(page);
			}
			else
				window.alert("当前是第一个");
		}
		pri.onclick=function(){
			window.alert("正在调用,请勿重复点击,否则会下载多份文件")
			createPDF(page);
		}

		function createPDF(page){
			$.ajax({
				url: 'GetPDF',
				type: 'get',
				cache: false,
				dataType: 'json',
				data: {
					'num': page
				}
			})
		}

		function createInfo(page){
			$.ajax({
				url:'getEntryInfo',
				type:'post',
				cache:false,
				dataType:'json',
				data:{
					'num':page
				},
				success:function(result){
					if(result.status=="success"){
						img.src="photo/"+result.info.imgURL;
						name.innerHTML=result.info.name,
								grade.innerHTML=result.info.grade,
								majorClass.innerHTML=result.info.className,
								born.innerHTML=result.info.birthday,
								hobby.innerHTML=result.info.hobby,
								phone.innerHTML=result.info.phone,
								choose.innerHTML=result.info.intent,
								resume.innerHTML=result.info.resume,
								experience.innerHTML=result.info.experience,
								futurePlan.innerHTML=result.info.plan
					}else if(result.status=="resultNull") {
						alert(result.message);
					}
					else
						alert(result.message);
					page--;
				},
				error:function(){
					alert("网络传输有问题，请检查网络连接或刷新页面再次进行操作");
				}
			})

		}
	}

</script>

</body>
</html>
